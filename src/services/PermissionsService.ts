...
class PermissionsService {
  ...
  public getCameraPermission(): Permission {
    return Platform.select({
      ios: 'ios.permission.CAMERA',
      android: 'android.permission.CAMERA',
      default: 'android.permission.CAMERA', // Fallback for other platforms
    }) as Permission;
  }
  ...
  public async isPermissionBlocked(permissionType: 'camera' | 'notification'): Promise<boolean> {
    try {
      let permission: Permission;
      if (permissionType === 'camera') {
        permission = this.getCameraPermission();
      } else if (permissionType === 'notification') {
        if (Platform.OS !== 'android') {
          return false; // Notifications only supported on Android for this app
        }
  ...
  public async getPermissionDetails(permissionType: 'camera' | 'notification'): Promise<{
    granted: boolean;
    blocked: boolean;
    unavailable: boolean;
  }> {
